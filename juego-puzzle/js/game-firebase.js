/**
 * PorkCasare Puzzle - Firebase Handler
 */
...
export class GameFirebaseHandler {
    ...
    return new Promise((resolve, reject) => {
      ...
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          if (!redirecting) {
            redirecting = true;
            window.location.href = '/distribuidor-login.html?redirect=/juego-puzzle/';
          }
        }
      });
    });
  ...
  async loadUserData() {
    ...
    if (userSnap.exists()) {
      ...
      if (!this.userData.puzzleProgress) {
        this.userData.puzzleProgress = this.getDefaultProgress();
        await this.saveProgress(this.userData.puzzleProgress);
      }
  ...
  getGameProgress() { return this.userData?.puzzleProgress || this.getDefaultProgress(); }
}
