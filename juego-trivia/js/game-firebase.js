/**
 * PorkCasare Trivia - Firebase Handler
 */
...
export class GameFirebaseHandler {
    ...
    return new Promise((resolve, reject) => {
      ...
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          if (!redirecting) {
            redirecting = true;
            window.location.href = '/distribuidor-login.html?redirect=/juego-trivia/';
          }
        }
      });
    });
  ...
  async loadUserData() {
    ...
    if (userSnap.exists()) {
      ...
      if (!this.userData.triviaProgress) {
        this.userData.triviaProgress = this.getDefaultProgress();
        await this.saveProgress(this.userData.triviaProgress);
      }
  ...
  loadMonthlyTracker() {
    ...
    if (tracker && tracker.month === currentMonth) {
      ...
      // Nuevo mes o no existe tracker - resetear
      this.monthlyCoinsEarned = 0;
    }
  }
  ...
  getGameProgress() { return this.userData?.triviaProgress || this.getDefaultProgress(); }
}
